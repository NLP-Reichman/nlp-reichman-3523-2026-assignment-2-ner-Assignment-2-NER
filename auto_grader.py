...
def test_NERNet_partial(results):
    f1 = results["f1"]
...
def test_NERNet(results):
    ...
    if f1 < 0.88:
        return f"F1 is {f1}, expected at least 0.88"
    if f1_wo_o < 0.66:
        return f"F1 without O is {f1_wo_o}, expected at least 0.66"
    return 1

def test_glove_partial(results):
    f1 = results["f1"]
    f1_wo_o = results["f1_wo_o"]
...
def test_glove(results):
    ...
    if f1 < 0.85:
        return f"F1 is {f1}, expected at least 0.85"
    if f1_wo_o < 0.64:
        return f"F1 without O is {f1_wo_o}, expected at least 0.64"
    return 1
...
def main():
    ...
    match args.test:
        ...
        case 'test_NERNet_partial':
            result = test_NERNet_partial(results["test_NERNet"])
        case 'test_NERNet':
            result = test_NERNet(results["test_NERNet"])
        case 'test_glove_partial':
            result = test_glove_partial(results["test_glove"])
        case 'test_glove':
